...
class ContactView(APIView):
    ...
    def get(self, request):
        ...
        if dep == 'OTHER':
            qs = qs.filter(Q(department__exact=dep), ~Q(
                contact_type__exact='EMERGENCY') & ~Q(contact_type__exact='OTHER'))
        ...
        if search is not None and search != '':
            if len(search.split(' ')) == 1:
                qs = qs.filter(Q(name__icontains=search) | Q(description__icontains=search) | Q(
                    phone_contact_set__phone__icontains=search))
            else:
                search_list = search.split(' ')
                qs = qs.filter(*[Q(name__icontains=word) | Q(description__icontains=word) | Q(
                    phone_contact_set__phone__icontains=word) for word in search_list])
...
class GeometryObjectView(APIView):
    def get(self, request, format=None):
        ...
        serializer = GeometryObjectSerializer(
            geometry_object_models, many=True)
        return Response(serializer.data)
